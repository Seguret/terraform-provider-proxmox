...
var _ datasource.DataSource = &NodeRRDDataSource{}
var _ datasource.DataSourceWithConfigure = &NodeRRDDataSource{}

type NodeRRDDataSource struct {
	client *client.Client
}

type RRDDataPointModel struct {
...
type NodeRRDDataSourceModel struct {
	ID        types.String        `tfsdk:"id"`
	NodeName  types.String        `tfsdk:"node_name"`
	Timeframe types.String        `tfsdk:"timeframe"`
	DataPoints []RRDDataPointModel `tfsdk:"data_points"`
}

func NewDataSource() datasource.DataSource {
	return &NodeRRDDataSource{}
}

func (d *NodeRRDDataSource) Metadata(_ context.Context, req datasource.MetadataRequest, resp *datasource.MetadataResponse) {
	resp.TypeName = req.ProviderTypeName + "_virtual_environment_node_rrd"
}

func (d *NodeRRDDataSource) Schema(_ context.Context, _ datasource.SchemaRequest, resp *datasource.SchemaResponse) {
	resp.Schema = schema.Schema{
		Description: "Retrieves RRD performance data for a Proxmox VE node.",
...
func (d *NodeRRDDataSource) Configure(_ context.Context, req datasource.ConfigureRequest, resp *datasource.ConfigureResponse) {
...
func (d *NodeRRDDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	var config NodeRRDDataSourceModel
	...
	tflog.Debug(ctx, "Reading node RRD data", map[string]any{"node": node, "timeframe": timeframe})

	points, err := d.client.GetNodeRRDData(ctx, node, timeframe)
	if err != nil {
		resp.Diagnostics.AddError("Error reading node RRD data", err.Error())
		return
	}

	state := NodeRRDDataSourceModel{
		ID:         types.StringValue(fmt.Sprintf("%s-rrd-%s", node, timeframe)),
		NodeName:   config.NodeName,
		Timeframe:  config.Timeframe,
		DataPoints: make([]RRDDataPointModel, len(points)),
	}

	for i, p := range points {
		state.DataPoints[i] = RRDDataPointModel{
