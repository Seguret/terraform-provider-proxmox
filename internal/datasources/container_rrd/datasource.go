...
var _ datasource.DataSource = &ContainerRRDDataSource{}
var _ datasource.DataSourceWithConfigure = &ContainerRRDDataSource{}

type ContainerRRDDataSource struct {
	client *client.Client
}

type ContainerRRDDataPointModel struct {
...
type ContainerRRDDataSourceModel struct {
	ID         types.String                 `tfsdk:"id"`
	NodeName   types.String                 `tfsdk:"node_name"`
	VMID       types.Int64                  `tfsdk:"vmid"`
	Timeframe  types.String                 `tfsdk:"timeframe"`
	DataPoints []ContainerRRDDataPointModel `tfsdk:"data_points"`
}

func NewDataSource() datasource.DataSource {
	return &ContainerRRDDataSource{}
}

func (d *ContainerRRDDataSource) Metadata(_ context.Context, req datasource.MetadataRequest, resp *datasource.MetadataResponse) {
	resp.TypeName = req.ProviderTypeName + "_virtual_environment_container_rrd"
}

func (d *ContainerRRDDataSource) Schema(_ context.Context, _ datasource.SchemaRequest, resp *datasource.SchemaResponse) {
	resp.Schema = schema.Schema{
		Description: "Retrieves RRD performance data for a Proxmox VE LXC container.",
...
func (d *ContainerRRDDataSource) Configure(_ context.Context, req datasource.ConfigureRequest, resp *datasource.ConfigureResponse) {
...
func (d *ContainerRRDDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	var config ContainerRRDDataSourceModel
	...
	tflog.Debug(ctx, "Reading container RRD data", map[string]any{"node": node, "vmid": vmid, "timeframe": timeframe})

	points, err := d.client.GetContainerRRDData(ctx, node, vmid, timeframe)
	if err != nil {
		resp.Diagnostics.AddError("Error reading container RRD data", err.Error())
		return
	}

	state := ContainerRRDDataSourceModel{
		ID:         types.StringValue(fmt.Sprintf("%s-%d-rrd-%s", node, vmid, timeframe)),
		NodeName:   config.NodeName,
		VMID:       config.VMID,
		Timeframe:  config.Timeframe,
		DataPoints: make([]ContainerRRDDataPointModel, len(points)),
	}

	for i, p := range points {
		state.DataPoints[i] = ContainerRRDDataPointModel{
